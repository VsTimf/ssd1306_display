...
/**
 * @brief Construct a new Bar Chart. Coordinates are correspond to left bottom corner of chart
 * 
 * @param _ds                         display segment to display chart
 * @param x_px                        x coordinate, px (left bottom chart corner)
 * @param y_px                        y coordinate, px (left bottom chart corner)
 * @param _qnt                        quantity of charts
 * @param w_px                        width of chart item, px
 * @param sp_px                       spacing between chart items, px
 * @param h_px                        max heigth of chart item, px                        
 * @param _bmin                       minimum input numer corresponds to zero chart level
 * @param _bmax                       maximum input number corresponds to "h_px" chart level
 */
BarChart::BarChart(DispSegment* _ds, uint8_t x_px, uint8_t y_px, uint8_t _qnt, uint8_t w_px, uint8_t sp_px, uint8_t h_px, signed _bmin, signed _bmax):
  ds(_ds), 
  x(x_px), y(y_px), 
  qnt(_qnt), w(w_px), sp(sp_px), h(h_px), 
...
/**
 * @brief Construct a new Bar Chart. Draw it in the left bottom corner of the cpecified segment.
 * 
 * @param _ds                         display segment to display chart
 * @param _qnt                        quantity of charts
 * @param w_px                        width of chart item, px
 ...
 * @param _bmax                       maximum input number corresponds to "h_px" chart level
 */
BarChart::BarChart(DispSegment* _ds, uint8_t _qnt, uint8_t w_px, uint8_t sp_px, uint8_t h_px, signed _bmin, signed _bmax)
{
  BarChart(_ds, 0, _ds->shp-1, _qnt, w_px, sp_px, h_px, _bmin, _bmax);
}
...
/**
 * @brief Construct a new Bar Chart. Fits the entire bar chart into a segment -> Automaticly calc width & max heigth of chart items
 * 
 * @param _ds                         display segment to display chart
 * @param _qnt                        quantity of charts
 * @param sp_px                       spacing between chart items, px
 ...
 * @param _bmax                       maximum input number corresponds to "h_px" chart level
 */
BarChart::BarChart(DispSegment* _ds, uint8_t _qnt, uint8_t sp_px, signed _bmin, signed _bmax) : 
  ds(_ds), 
  x(0), y(_ds->shp-1), 
  qnt(_qnt), w((_ds->sw-(_qnt-1)*sp_px) / _qnt), sp(sp_px), h( _ds->shp), bmin(_bmin), bmax(_bmax)
  {          
    lv = new uint8_t[qnt];
...
void BarChart::show(signed* nv, bool part_update)
{
  ...
  for(uint8_t ch = 0; ch < qnt; ch++)
  {
    nv_px = ((nv[ch] - bmin) * h) / (bmax - bmin);
  ...
  {
    if(part_update)
      ds->update_part(x, max_val, x+(w+sp)*qnt-sp-1, min_val);
    else
      ds->update_part(x, y+1-h, x+(w+sp)*qnt-sp-1, y);
  }
}